...
func (repo *StoreProvider) GetOne(uuid string) (Event, error) {
	event := Event{}
	sql := `SELECT uuid,
                   user_id,
                   type,
...
func (repo StoreProvider) GetOneByUserId(userId string) (Event, error) {
	event := Event{}
	sql := `SELECT uuid,
                   user_id,
                   type,
	...
	row := repo.db.QueryRow(sql, userId)
	err := row.Scan(&event.Uuid, &event.UserId, &event.Type, &event.Status, &event.Caption, &event.Message, &event.IsSeen)
